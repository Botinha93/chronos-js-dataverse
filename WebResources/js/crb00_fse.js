...
function CriteriosPadrao(executionContext) {
    class entity {
        constructor(contrato, status, fisico, financeiro) {
...
async function CalcularPred(ids) {
    Xrm.Utility.showProgressIndicator("Atualizando datas");
    var arrayPred = new Map();
    for (let index = 0; index < ids.length; index++) {
        const id = ids[index];
        var pred = { valor: await Xrm.WebApi.retrieveRecord("crb00_newtable2", id), percorrido: false, update: {} };
        console.log(pred);
...
function recursivePred(anterior, arrayPred) {
    if (!arrayPred.get(anterior).percorrido) {
        var predecessoes = arrayPred.get(anterior).valor["crb00_predecessao"].split(',').map((item) => item.replace(/\D/g, ''));
...
function updateAndamento(executionContext) {
    var formContext = executionContext.getFormContext();
    Xrm.WebApi.retrieveRecord("crb00_tipodedocumentofse", formContext.getAttribute("crb00_documentofse").getValue()[0].id.replace(/[{}]/g, "")).then(function (fse) {
...
function documentoFSE(executionContext) {
    var formContext = executionContext.getFormContext();
    Xrm.WebApi.retrieveRecord("crb00_tipodedocumentofse", formContext.getAttribute("crb00_documentofse").getValue()[0].id.replace(/[{}]/g, "")).then(function (fse) {
        formContext.getAttribute("crb00_codigofse").setValue(fse["crb00_sigla"]);
        formContext.getAttribute("crb00_duracao").setValue(fse["crb00_duracao"]);
...
function gerarBM(primaryControl) {
    var formContext = primaryControl;
    var accountName = formContext.getControl("name");
}
